...
export async function ProductView({
  ...
  const hasRichData =
    hasImages || hasSpecs || hasForms || product.shortDescription;
        ...
        <div className="mt-8 grid gap-12 lg:grid-cols-[1.3fr_1fr] lg:gap-16">
          <div>
            ...
            <dl className="mt-8 grid grid-cols-2 gap-x-6 gap-y-5 border-t border-traya-border pt-6 sm:max-w-md">
              ...
              <Fact label={t("detail.pricingLabel")}>
                {t("detail.pricing")}
              </Fact>
            </dl>
              ...
              <div className="mt-8 rounded-2xl border border-traya-border bg-traya-surface p-6">
                ...
                <dl className="mt-4 space-y-3">
                  {product.specifications!.map((spec, i) => (
                    <div
                      key={i}
                      className="flex justify-between gap-4 border-b border-traya-border pb-3 last:border-0 last:pb-0"
                    >
            ...
            <div className="mt-6">
              {product.brochureUrl ? (
                ...
                >
                  <Download
                    className="size-4 text-traya-saffron-lo"
                    aria-hidden="true"
                  />
            ...
            {hasForms && (
              <div className="mt-8 space-y-4">
                <h2 className="font-display text-lg text-foreground">
                  {t("detail.availableForms")}
                </h2>
                {product.forms!.map((form) => (
                  <div
                    key={form._key}
                    className="rounded-xl border border-traya-border bg-traya-surface p-4"
                  >
        ...
        {related.length > 0 && (
          <div className="mt-24 border-t border-traya-border pt-12">
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
              <div>
                <p className="text-sm font-medium text-traya-red-deep mb-2">
                  {category.title}
                </p>
