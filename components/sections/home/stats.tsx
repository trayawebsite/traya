...
// a mid-animation frame was showing "42%" etc. A thin vermilion top rule ties
// it to the brand.
const STATS = [
    ...
    <section className="relative bg-traya-deep text-traya-cream">
      ...
      <Container className="py-12 lg:py-16">
        <dl className="grid grid-cols-2 gap-y-12 sm:grid-cols-4 sm:gap-x-6 sm:divide-x sm:divide-traya-cream/10">
          {STATS.map((s) => (
            <div key={s.key} className="px-2 text-center sm:px-4">
              <dt className="font-display text-5xl leading-none text-traya-saffron lg:text-6xl">
                {s.value}
              </dt>
              <dd className="mx-auto mt-3 max-w-[14ch] text-xs uppercase tracking-[0.16em] text-traya-cream/55">
                {t(`stats.${s.key}`)}
              </dd>
            </div>
          ))}
