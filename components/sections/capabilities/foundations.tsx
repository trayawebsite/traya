...
export async function Foundations({data}: {data?: FeatureItem[]}) {
  ...
  const items = data && data.length > 0
    ? data.map((item) => ({title: item.title, description: item.description ?? ''}))
    : FOUNDATIONS.map((f) => ({title: t(`${f}Title`), description: t(`${f}Body`)}));
    ...
    <section className="bg-traya-deep text-traya-cream">
      <Container className="py-section-lg">
        ...
        <div className="mt-12 grid gap-x-8 gap-y-10 sm:grid-cols-2 lg:grid-cols-4">
          {items.map((item, i) => (
            <div key={i} data-stagger className="border-t-2 border-traya-saffron/40 pt-5">
              <h3 className="font-display text-lg text-traya-cream">{item.title}</h3>
              <p className="mt-1.5 text-sm leading-relaxed text-traya-cream/70">{item.description}</p>
            </div>
          ))}
        </div>
      </Container>
