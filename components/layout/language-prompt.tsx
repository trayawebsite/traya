...
function isDismissed(): boolean {
  ...
  return document.cookie.includes(`${COOKIE_NAME}=`) || sessionStorage.getItem(PROMPT_DISMISSED) === '1';
}
...
export function LanguagePrompt() {
  ...
  return (
    <div className="fixed inset-x-0 top-0 z-[100] flex items-center justify-center gap-3 bg-traya-deep px-4 py-2.5 text-sm text-traya-cream shadow-lg">
      <span>
        {suggested === 'ar' ? 'هل تفضل المتابعة بالعربية؟' : 'Continuer en Français ?'}
      </span>
      <button
      ...
      <button
        onClick={dismiss}
        aria-label={suggested === 'ar' ? 'إغلاق' : suggested === 'fr' ? 'Fermer' : 'Dismiss'}
        className="ml-1 rounded-md p-1 text-traya-cream/60 transition-colors hover:text-traya-cream focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-traya-cream"
      >
